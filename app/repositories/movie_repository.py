...
class MovieRepository:
    ...
    def filter_actors(actor: str, watched_tconsts: List[str], length: int, limit: int) -> list:
        return (db.principals.find({"nconst": actor, "tconst": {"$nin": watched_tconsts}}, {"tconst": 1})
                .sort("tconst", -1).limit(limit - length))
    ...
    def filter_movies_by_genre(genre: str, watched_tconsts: List[str], limit: int) -> List[Movie]:
        return list(
            db.titles.find({"genres": {"$regex": genre, "$options": "i"}, "tconst": {"$nin": watched_tconsts}, })
