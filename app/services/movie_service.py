...
class MovieService:
    ...
    def get_all_movies(title: str, page: int, limit: int) -> List[Movie]:
        ...
        movies: List[Movie] = MovieRepository.get_movies(skip, filter_query, limit)
        for movie in movies:
    ...
    @staticmethod
    def get_directors_and_actors_movies(watched_titles: List[dict], high_rated: List[str], limit: int) -> Union[List[Movie], None]:
        ...
        for director in directors:
            director_movies: list = MovieRepository.filter_crew_by_directors(director, watched_tconsts, limit)
            for movie in director_movies:
                movie_id: str = movie["tconst"]
                if movie_id not in rec:
                    rec.append(movie_id)
        ...
        if len(rec) < limit:
            ...
            # buscar por ator
            for actor in actors:
                actors_movies: list = MovieRepository.filter_actors(actor, watched_tconsts,len(rec), limit)
                for movie in actors_movies:
                    movie_id = movie["tconst"]
                    if movie_id not in watched_tconsts and movie_id not in rec:
                        rec.append(movie_id)
                        if len(rec) >= limit: break
                if len(rec) >= limit: break
    ...
    List[Movie]:
        ...
        for title in watched_titles:
            tconst: str = title.get("tconst")
            user_rating: float = rating_by_tconst.get(tconst)
            if not user_rating:
    ...
    @staticmethod
    def get_recommended_movies(usuario_id: ObjectId, limit: int) -> Union[List[Movie], None]:
        """
